...
export const createBudget = async (req, res) => {
  ...
  if (!categoryId || !amount || !startDate || !endDate) {
    return errorResponse(res, 400, 'All fields are required: categoryId, amount, startDate, endDate');
  }
  ...
  try {
    ...
    if (overlappingBudget) {
      return errorResponse(res, 400, 'A budget for this category already exists in the specified date range');
    }
    ...
    await budget.save();
    await budget.populate('categoryId', 'name type icon');
...
export const updateBudget = async (req, res) => {
  ...
  try {
    ...
    if (!existingBudget) {
      return errorResponse(res, 404, 'Budget not found or does not belong to user');
    }
  ...
  try {
    ...
    const budgetSummary = await Promise.all(
      activeBudgets.map(async (budget) => {
        ...
        return {
          ...
          percentageUsed: budget.amount > 0 ? (spent / budget.amount * 100).toFixed(2) : 0
        };
      })
    );
    ...
    const overviewData = {
      ...
      summary: {
        ...
        overallPercentageUsed: totalBudgeted > 0 ? (totalSpent / totalBudgeted * 100).toFixed(2) : 0,
